...
var pizzaElementGenerator = function(i) {
  ...
  pizzaImage.src = "images/pizza.232x.min.png";
...
var resizePizzas = function(size) {
  ...
  // For optimization, functions changeSliderLabel, determineDX, sizeSwitcher, & changePizzaSizes
  // were replaced by the following code segment: 
  // Change the value for the size of the pizza above the slider
  var sizeIndex = Number(size) - 1;
  var sizes = ['Small', 'Medium', 'Large'];
  var newSize = sizes[sizeIndex];
  document.getElementById("pizzaSize").innerHTML = newSize;  
  // Determine the new width for pizza images
  var widths = ['25%', '33.33%', '50%'];
  var newWidth = widths[sizeIndex]; 
  // Rezize the pizza images
...
function updatePositions() {
  ...
  //}
  // For optimization the following items were moved outside the loop:
  //   document.body.scrollTop and part phase calculation
  ...
  for (var i = 0; i < elementsLength; i++) {
    phase = Math.sin((scrollTopTerm) + (i % 5));
    elements[i].style.left = elements[i].basicLeft + 100 * phase + 'px';
  }
